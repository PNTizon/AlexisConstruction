    ...
    public partial class BillingStatementForm : Form
    {
        ...
        public void LoadBillingReport(string clientName)
        {
            using (SqlConnection conn = new SqlConnection(Connection.Database))
            {
                ...
                string query = @"SELECT bk.BookingID, c.Firstname + ' ' + c.Lastname AS ClientName,c.ContactNumber,c.Address, 
                                        bk.BillingDate,bk.BookedDate,bk.TotalAmount, bk.PaymentStatus
                                FROM Booking bk
        ...
        private void LoadBillingByDate(DateTime startDate, DateTime endDate)
        {
            using (SqlConnection conn = new SqlConnection(Connection.Database))
            {
                conn.Open();
                string query = @" SELECT bk.BookingID, c.Firstname + ' ' + c.Lastname AS ClientName, c.ContactNumber,c.Address,
                                        bk.BillingDate, bk.BookedDate,bk.TotalAmount, bk.PaymentStatus, bk.PaymentMethod , s.ServiceName
                                FROM Booking bk
                                JOIN Clients c ON bk.ClientID = c.ClientID
                                JOIN Services s ON bk.ServiceID = s.ServiceID
